...
public class DepositDAO {
	 ...
	 public static List<DepositBean> getFilteredDeposits(String whereClause, int start, int limit) {
	  ...
	  public static int totalCount() {
			 ...
			 try {
				 connection = DBUtil.provideConnection();
			   String query = "SELECT COUNT(*) AS count FROM (SELECT userID, DATE(transactiondate) AS transaction_date, Amount, MAX(status) AS status FROM trading.transaction WHERE Transactiontype = 'Deposit' GROUP BY userID, DATE(transactiondate), Amount) AS test";
		    ...
		    try {
		        ...
		        if ("Admin".equals(username)) {
		            ...
		            String adminQuery = "SELECT TransactionID, transactiondate, Amount, userID, MAX(status) AS status "
		        		    ...
		        		    + "GROUP BY TransactionID, transactiondate, Amount, userID, date(transactiondate), amount "
		    ...
		    try {
		        ...
		        if (userResultSet.next()) {
		            ...
		            String countQuery = "SELECT COUNT(*) AS count " +
		                    "FROM (SELECT TransactionID, MAX(transactiondate) AS transactiondate, " +
		                    "             MAX(Amount) AS Amount, userID, MAX(status) AS status " +
		                    "      FROM trading.transaction " +
		                    "      WHERE Transactiontype='Deposit' AND userID=? " +
		                    "      GROUP BY userID, DATE(transactiondate), Amount, TransactionID) AS test";
