	...
	//WithdrawalID, userID, AccountID, WithdrawalDate, Amount
	 public static List<WithdrawalBean> getFilteredWithdrawals(String whereClause, int start, int limit) {
	  ...
	  public static int totalCount() {
			 ...
			 try {
				 connection = DBUtil.provideConnection();
				 String query = "SELECT COUNT(*) AS count FROM (SELECT MAX(TransactionID) AS TransactionID, DATE(transactiondate) AS transaction_date, Amount, userID, MAX(status) AS status FROM trading.transaction WHERE Transactiontype='Withdrawal' GROUP BY userID, DATE(transactiondate), amount) AS test";
	  ...
	  public static List<TransactionBean>getWithdrawalsByUsername(String username) {
		    ...
		    try {
		        ...
		        if ("Admin".equals(username)) {
		        	...
		        	String adminQuery = "SELECT MAX(TransactionID) AS TransactionID, "
	                		...
	                		+ "GROUP BY userid, Transactiontype, DATE(transactiondate)"
	                		+ "ORDER BY transactiondate DESC, MAX(Amount);";
		depositStatement = connection.prepareStatement(adminQuery);
		            ...
		            if (userResultSet.next()) {
		                ...
		                String depositQuery = "SELECT MAX(TransactionID) AS TransactionID, "
		                		+ "       DATE(transactiondate) AS transactiondate, "
		                		+ "       MAX(Amount) AS Amount, "
		                		+ "       userID, "
		                		+ "       MAX(status) AS status "
		    ...
		    try {
		        ...
		        if (userResultSet.next()) {
		            ...
		            String countQuery = "SELECT COUNT(*) AS count "
		            		+ "FROM (SELECT MAX(TransactionID) AS TransactionID,  transactiondate,  Amount, "
		            		+ " userID, MAX(status) AS status FROM trading.transaction "
		            		+ "    WHERE Transactiontype = 'Withdrawal' AND userid = ? "
		            		+ "    GROUP BY userid, DATE(transactiondate), amount, transactiondate "
		            		+ ") test";
